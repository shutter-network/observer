...
func (tr *TransactionRepo) UpdateTransaction(ctx context.Context, tx *TransactionV1) (*TransactionV1, error) {
	rows := tr.GetDB(ctx).QueryRow(ctx, `
		...
		RETURNING
			...
			block_hash,
			created_at,
			updated_at`, tx.ID, tx.EncryptedTx, tx.DecryptionKey, tx.Slot, tx.BlockHash)

	err := rows.Scan(&tx.ID, &tx.EncryptedTx, &tx.DecryptionKey, &tx.Slot, &tx.BlockHash, &tx.CreatedAt, &tx.UpdatedAt)
	if err != nil {
