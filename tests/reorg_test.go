...
func (s *TestMetricsSuite) TestHandlePotentialReorgValidatorRegistry() {
	ctx := context.Background()
	// Setup initial sync status
	blockHash := [32]byte{1, 2, 3}
	err := s.dbQuery.CreateValidatorRegistryEventsSyncedUntil(ctx, data.CreateValidatorRegistryEventsSyncedUntilParams{
		BlockHash:   blockHash[:],
		BlockNumber: 99,
	})
	s.Require().NoError(err)
	...
	// no reorg
	err = s.validatorRegistrySyncer.HandlePotentialReorg(ctx, header)
	s.Require().NoError(err)
	...
	syncStatus, err := s.dbQuery.QueryValidatorRegistryEventsSyncedUntil(ctx)
	s.Require().NoError(err)
	s.Require().Equal(int64(89), syncStatus.BlockNumber)
	s.Require().Empty(syncStatus.BlockHash)
}
