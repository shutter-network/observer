...
EXPOSE 23003
ENTRYPOINT ["./observer"]
CMD ["start", "--rpc-url", "$RPC_URL", "--beacon-api-url", "${BEACON_API_URL}", "--sequencer-contract-address", "$SEQUENCER_CONTRACT_ADDRESS", "--validator-registry-contract-address","$VALIDATOR_REGISTRY_CONTRACT_ADDRESS", "--p2pkey", "$P2P_KEY", "--inclusion-delay", "$INCLUSION_DELAY"]
